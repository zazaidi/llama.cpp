...
struct ggml_tensor * llama_model_loader::borrow_shared_tensor(const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne) {
    // Only draft heads can omit these tensors.
    ...
    // Prefer a tensor included in the draft file.
    ...
    // The draft uses the target tensor directly.
    size_t dim = 0;
    ...
    // The target model owns this tensor.
    return src;
}
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    // Borrow before dimension checks and tied-output fallback.
