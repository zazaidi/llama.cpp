...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    // Read this before sizing per-layer arrays.
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // The MTP block uses the trunk's full-attention and MoE path.
        ...
        // eh_proj fuses the embedding and hidden projections.
        ...
        // Collapse the MTP streams and apply the output norm.
        ...
        // Dedicated embeddings and output are optional.
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Export all hidden rows before gathering unmasked MTP output.
    ...
    // Export the combine result because a reshape view has no scheduler backend.
    if (cparams.embeddings_nextn) {
        cb(res_hc, "h_nextn", -1);
        res->t_h_nextn = res_hc;

        // Gather output rows after exporting the unmasked hidden state.
...
// Qwen4 MTP uses one dense-attention draft block and exports its wide residual.
// Dense attention is verified by the target; QSA can be added for long-context fidelity.
...
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) :
    ...
    // Normalize each hyper-connection stream before applying the head scale.
    ...
    // Broadcast the token embedding across hyper-connection streams.
    ...
    // Apply the fused embedding and hidden projection to each stream.
    ...
    // Export the owned wide stream for the next draft step.
    cb(res_hc, "h_nextn", -1);
    res->t_h_nextn = res_hc;

    // Collapse the streams and apply the output norm.
    cur = build_hc_mix(res_hc,
            layer.nextn.hc_head_norm, layer.nextn.hc_head_down, layer.nextn.hc_head_up,
            nullptr, nullptr, -1);
    cb(cur, "mtp_hc_head", -1);

    // The driver reads the n_embd_out-wide t_h_nextn instead of t_embd.
